...
st.set_page_config(layout="wide", page_title="NYC Uber Ridesharing", page_icon=":taxi:")
...
def filterdata(data, hour_selected):
    return data[data["date/time"].dt.hour == hour_selected]
...
@st.experimental_memo
def histdata(df, hr):
    filtered = df[(df["date/time"].dt.hour >= hr) & (df["date/time"].dt.hour < (hr + 1))]
